...
static async insert({ name, author, genre, completed }) {
  const { rows } = await pool.query('INSERT INTO books (name, author, genre, completed) VALUES ($1, $2, $3, $4) RETURNING *', [name, author, genre, completed]);
...
static async updateById(id, { name, author, genre, completed }) {
  ...
  const { rows } = await pool.query('UPDATE books SET name=$1, author=$2, genre=$3, completed=$4 WHERE id=$5 RETURNING *', [newName, newAuthor, newGenre, newCompleted, id]);
